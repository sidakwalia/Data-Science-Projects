...
while(cap.isOpened()):
    ...
    mask1 = cv2.morphologyEx(mask1, cv2.MORPH_OPEN, np.ones((3,3),np.uint8),iterations=2)  ## for removing the noise from the image
    mask1 = cv2.dilate(mask1,np.ones((3,3),np.uint8),iterations = 1) # for smoothening of the image
